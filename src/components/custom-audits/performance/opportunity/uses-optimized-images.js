...
export const UsesOptimizedImages = ({ lighthouse }) =>
  lighthouse.audits['uses-optimized-images'].score !== 1 && (
    <li id="uses-optimized-images">
      <dl>
        <dt>
          <Markdown>
            {lighthouse.audits['uses-optimized-images'].title}
          </Markdown>
          <span
            className={`${
              lighthouse.audits['uses-optimized-images'].score < 0.5
                ? `bg-red-300 text-red-900`
                : `bg-orange-300 text-orange-900`
            } font-black inline-block ml-2 px-2 rounded-full text-sm whitespace-no-wrap`}
          >
            {lighthouse.audits['uses-optimized-images'].displayValue}
          </span>
        </dt>
        <dd>
          <details>
